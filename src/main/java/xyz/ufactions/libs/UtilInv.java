...
public class UtilInv {
    ...
    public static void DisallowMovementOf(InventoryClickEvent event, String name, Material type, byte data, boolean inform, boolean allInventorties) {
        ...
        if (event.getAction() == InventoryAction.HOTBAR_SWAP ||
            ...
            if (inform)
                event.getWhoClicked().sendMessage(F.main("Inventory", "You cannot hotbar swap " + F.elem(name) + "."));
            ...
            if (inform)
                event.getWhoClicked().sendMessage(F.main("Inventory", "You cannot move " + F.elem(name) + "."));
            event.setCancelled(true);
        }
    }
